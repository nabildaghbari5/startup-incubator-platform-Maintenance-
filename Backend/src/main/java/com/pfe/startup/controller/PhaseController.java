...
@RestController
@RequestMapping("/api/incubateur/{incId}/phases")
...
public class PhaseController {
    ...
    // GET /api/incubateur/{incId}/phases
    @GetMapping
    public ResponseEntity<List<Map<String, Object>>> list(@PathVariable Long incId) {
        List<Phase> phases = phaseRepo.findByIncubateurIdOrderByNumeroAsc(incId);
        return ResponseEntity.ok(phases.stream().map(this::toMap).collect(Collectors.toList()));
    }

    // POST /api/incubateur/{incId}/phases
    @PostMapping
    public ResponseEntity<?> create(@PathVariable Long incId,
                                    @RequestBody Map<String, Object> body) {
        // Chercher l'user (incubateur) par son id
        User incubateur = userRepo.findById(incId).orElse(null);
        if (incubateur == null) {
            return ResponseEntity.badRequest().body(Map.of("error", "Incubateur introuvable id=" + incId));
        }

        Phase phase = Phase.builder()
                .titre(str(body, "titre"))
                .mois(str(body, "mois"))
                .icone(str(body, "icone") != null ? str(body, "icone") : "📌")
                .description(str(body, "description"))
                .couleur(str(body, "couleur") != null ? str(body, "couleur") : "#ec4899")
    ...
    // PUT /api/incubateur/{incId}/phases/{id}
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Long incId,
                                    @PathVariable Long id,
                                    @RequestBody Map<String, Object> body) {
        Phase phase = phaseRepo.findById(id).orElse(null);
        if (phase == null) return ResponseEntity.notFound().build();

        if (body.containsKey("titre"))       phase.setTitre(str(body, "titre"));
        if (body.containsKey("mois"))        phase.setMois(str(body, "mois"));
        if (body.containsKey("icone"))       phase.setIcone(str(body, "icone"));
        if (body.containsKey("description")) phase.setDescription(str(body, "description"));
        if (body.containsKey("couleur"))     phase.setCouleur(str(body, "couleur"));
        if (body.containsKey("numero"))      phase.setNumero(toInt(body.get("numero")));

        phase = phaseRepo.save(phase);
        return ResponseEntity.ok(toMap(phase));
    }

    // DELETE /api/incubateur/{incId}/phases/{id}
    @DeleteMapping("/{id}")
    ...
    // PATCH /api/incubateur/{incId}/phases/{id}/move?direction=up|down
    @PatchMapping("/{id}/move")
    public ResponseEntity<?> move(@PathVariable Long incId,
                                  @PathVariable Long id,
        ...
        List<Map<String, Object>> updated = phaseRepo
